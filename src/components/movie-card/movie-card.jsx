...
export default class MovieCard extends Component {
  ...
  formatText = (text, title) => {
    const endChar = title.length < 21 ? 210 : 120;
  ...
  ratedFilm = (rating) => {
    this.props.ratingFilm(rating);
  };

  getTags = (filmGenres, genres) => {
    return filmGenres.map((genre, idx) => {
      if (idx > 1) return null;
      const nameTag = genres.find((gen) => gen.id === genre.id).name;
  ...
  render() {
    const { id, title, description, imgUrl, dateRelease, genres, ratingCur, voteAverage } = this.props.movie;
          ...
          return (
            <Card key={id} className="movie-card" hoverable bordered={false} bodyStyle={this.style}>
              ...
              <div className="movie-card__info">
                ...
                <div className="movie-card__tags">{this.getTags(genres, gens)}</div>
                <Paragraph className="movie-card__description">{this.formatText(description, title)}</Paragraph>
                <Rate
                  style={{ fontSize: '15px' }}
                  count={10}
                  value={ratingCur > 0 ? ratingCur : null}
                  onChange={this.ratedFilm}
                  allowHalf
                />
              </div>
            </Card>
